...
const storageService = {
  ...
  async remove(storagePath, { bucket }) {
...
// Materials reference a real subjects row (subject_id, not a free-text
// column) — this looks up an existing subject by name within the given
// department/semester, or creates one, so the upload form can stay a
// plain text field without the insert failing the not-null FK.
async function resolveSubjectId({ departmentId, semesterId, subjectName }) {
  const name = (subjectName || "").trim();
  if (!name || !departmentId || !semesterId) return null;

  let resolvedSemesterId = semesterId;
  if (typeof semesterId === "string" && semesterId.startsWith("sem-")) {
    const semesterNumber = semesterId.replace("sem-", "");
    try {
      const semesters = await supabaseRest.get("semesters", `department_id=eq.${departmentId}&semester_number=eq.${semesterNumber}&limit=1`);
      resolvedSemesterId = semesters?.[0]?.id || null;
    } catch (err) {
      console.warn("Semester lookup failed:", err.message);
  ...
  try {
    const existing = await supabaseRest.get(
      "subjects",
      `department_id=eq.${departmentId}&semester_id=eq.${resolvedSemesterId}&name=ilike.${encodeURIComponent(name)}&limit=1`
    );
    if (existing && existing[0]) return existing[0].id;
  } catch (err) {
    console.warn("Subject lookup failed:", err.message);
  }

  try {
    const created = await supabaseRest.post("subjects", {
    ...
    console.warn("Subject creation failed:", err.message);
    return null;
  }
}
...
export const academicsService = {
   ...
   * 3. GET /subjects
   */
  async getSubjects(departmentId, semesterId) {
    if (!departmentId) return { ok: true, data: [] };

    if (isSupabaseConfigured()) {
      try {
        const query = semesterId
          ? `department_id=eq.${departmentId}&semester_id=eq.${semesterId}&order=name.asc`
          : `department_id=eq.${departmentId}&order=name.asc`;
        const rows = await supabaseRest.get("subjects", query);
        if (rows && Array.isArray(rows)) return { ok: true, data: rows };
      } catch (err) {
  ...
  async getMaterials(filters = {}) {
    ...
    const subjFilter = filters.subject_id || "";
  ...
  async uploadMaterial(payload) {
    ...
    if (supabase) {
      ...
      if (authError || !authData.user) {
        return { ok: false, error: "Please sign in again before uploading study material." };
      }
    ...
    if (typeof FormData !== "undefined" && payload instanceof FormData) {
      ...
      subjectIdOverride = payload.get("subject_id") || "";
      ...
      subjectIdOverride = payload.subject_id || "";
    ...
    if (isSupabaseConfigured()) {
      try {
        const subjectId = subjectIdOverride || (await resolveSubjectId({
          departmentId,
          semesterId,
        ...
        if (!subjectId) {
          await storageService.remove(finalStoragePath, { bucket: "academic_materials" }).catch(() => {});
          return { ok: false, error: "Could not resolve a subject for this material. Please check the department and semester and try again." };
        }
        ...
        await storageService.remove(finalStoragePath, { bucket: "academic_materials" }).catch(() => {});
        return { ok: false, error: `Could not save the material record: ${err.message}` };
      }
    }
    ...
    if (isSupabaseConfigured()) {
      try {
        const rows = await supabaseRest.get(
          "academic_materials",
          `uploaded_by=eq.${activeUserId}&select=*,departments(id,name,code),semesters(id,semester_number,name),subjects(id,name,code)&order=created_at.desc&limit=50`
        );
        const normalized = (rows || []).map((r) => ({
          id: r.id,
          title: r.title,
          ...
          semesterName: r.semesters?.name || (r.semesters?.semester_number ? `Sem ${r.semesters.semester_number}` : "Sem 1"),
          subject_id: r.subject_id || null,
          subject: r.subjects?.name || "General",
          type: r.material_type,
          material_type: r.material_type,
  ...
  async adminApproveDepartmentRequest(requestId, adminNote = "") {
    ...
    if (isSupabaseConfigured()) {
      try {
        ...
        if (req) {
          const insertedDept = await supabaseRest.post("departments", {
            name: req.name,
            code: req.code,
            active: true,
          });
          const newDept = Array.isArray(insertedDept) ? insertedDept[0] : insertedDept;
  ...
  async adminAddDepartment(deptData = {}) {
    ...
    if (isSupabaseConfigured()) {
      try {
        const inserted = await supabaseRest.post("departments", {
          name: name.trim(),
          code: code.trim().toUpperCase(),
          active: true,
        });
        const newDept = Array.isArray(inserted) ? inserted[0] : inserted;
  ...
  async adminDeleteDepartment(departmentId) {
    ...
    demoDeptStore = demoDeptStore.filter((d) => d.id !== departmentId);
    return { ok: true, message: "Department deleted successfully" };
  },
};